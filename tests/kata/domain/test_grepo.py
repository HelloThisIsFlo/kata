...
def repo(mock_api, thread_pool_executor):
    return GRepo(mock_api, thread_pool_executor)
...
class TestScenarios:
    ...
    def repo_with_scenario(self, repo):
        class RepoWithScenario:
            ...
            def get_file_urls(self, path):
                return repo.get_file_urls(user=NOT_USED, repo=self.scenario, path=path)
    ...
    def test_only_files(self, repo_with_scenario):
        ...
        result = repo_with_scenario.get_file_urls(path='')
    ...
    def test_nested_directories_path_isn_t_root(self, repo_with_scenario):
        ...
        # When: Fetching the root path: ''
        result = repo_with_scenario.get_file_urls(path='dir_at_root/dir_at_level_1')
